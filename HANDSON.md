...
* 学習データには **締切（knowledge cutoff）** があり、それ以降の出来事や新しいライブラリは知らない → 必要なら Web 検索やドキュメントを読ませて補う
* モデル自身に **セッションをまたぐ記憶はない**。知っているのは今コンテキストに入っている内容だけ → プロジェクトの前提は CLAUDE.md やメモリで毎回渡す（4章）
* Anthropic 社のモデルファミリー。上位ほど賢いが、トークン単価と応答時間も増える:
...
### 付録A：コンテキスト管理 — /context・/compact・/clear の使い分け
