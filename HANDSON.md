...
| コード以外の資料 | 仕様書 PDF、議事録、顧客要件 | claude.ai の Projects のナレッジ（付録C の TIPS 参照） | チャットで質問したとき |
...
> 同じ情報が Wiki・古い仕様書・人が要約したメモなど複数の場所にあると、Claude はどれを信じてよいか分からず、古い方を根拠にすることがある。「この情報の正はここ」を決めて INDEX.md にも明記しておく。**SSOT が明確であれば、Claude Code がそこを直接読み取って情報を抽出・整理できる** ため、人間が事前に整理した要約を使うより正確なデータが得られる場合がある（要約は作った時点で古くなり、抜け漏れや解釈も混ざる）。派生資料（要約・図・議事録）には原本の場所を書き、整理そのものは「原本から〇〇の観点で整理して」と Claude に任せ、人は原本の鮮度と正しさを保つことに集中する。
...
> 💡 **TIPS：「Projects」は2つある — 名前が同じなので分けて考える**
> **claude.ai の Projects** は、チャットのためのワークスペース（ナレッジファイル＋カスタム指示＋チャット履歴をひとまとめにし、Team / Enterprise ではチームで共有できる。全プランで利用可）。一方、この付録で扱う **Claude Code の Projects** は、claude.ai/code やデスクトップアプリの Code タブで **コーディネーターが複数のクラウドスレッドを並列に動かして PR まで進める** ための機能（Pro / Max のパブリックベータ）。同じ「Projects」でも別物なので、会話や資料でどちらを指しているかを明確にする。仕様書や議事録などコード以外の資料をチームで参照したいだけなら前者、実装タスクを並列に回したいなら後者。
...
* [Claude ヘルプセンター — What are projects?（claude.ai の Projects）](https://support.claude.com/en/articles/9517075-what-are-projects)
